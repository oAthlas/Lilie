...
def add_message_to_ui(app, sender, message, role):
    ...
    if role == "user":
        ...
        lilie_font_size = 22  # ajuste aqui o tamanho desejado
        msg_label = ctk.CTkLabel(
            app.chat_frame,
            text=f"{sender}: {message}",
            font=ctk.CTkFont(family=font_family, size=lilie_font_size),
            text_color=fg_color,
            anchor="w",
            justify="left",
...
def update_lilie_labels_wraplength(app):
    # Define o wraplength como a largura do chat_frame menos o padding
